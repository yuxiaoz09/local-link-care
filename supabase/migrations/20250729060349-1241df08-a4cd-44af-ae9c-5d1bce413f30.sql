-- Fix EXTRACT function compatibility issues and add debugging for authentication
...
BEGIN
  ...
  SELECT 
    ...
    -- Fix EXTRACT function - use proper interval calculation
    COALESCE(EXTRACT(DAY FROM (CURRENT_DATE - MAX(a.start_time::DATE)))::numeric, 999) as days_since_last_visit,
    COALESCE(SUM(a.price), 0) as customer_lifetime_value,
    -- RFM Score calculations - fix EXTRACT usage
    CASE 
      WHEN COALESCE(EXTRACT(DAY FROM (CURRENT_DATE - MAX(a.start_time::DATE)))::numeric, 999) <= 30 THEN 5
      WHEN COALESCE(EXTRACT(DAY FROM (CURRENT_DATE - MAX(a.start_time::DATE)))::numeric, 999) <= 60 THEN 4
      WHEN COALESCE(EXTRACT(DAY FROM (CURRENT_DATE - MAX(a.start_time::DATE)))::numeric, 999) <= 90 THEN 3
      WHEN COALESCE(EXTRACT(DAY FROM (CURRENT_DATE - MAX(a.start_time::DATE)))::numeric, 999) <= 180 THEN 2
      ELSE 1
